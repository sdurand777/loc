#!/usr/bin/env python3
"""
Script pour détecter les loop closures avec MegaLoc et visualiser les matches.
Génère des images composites montrant les paires query-loop avec les correspondances visuelles.

Usage:
    python visualize_loop_matches.py --images_path /path/to/images --output_dir loop_matches
"""
...
def extract_features_and_patches(model, img_tensor, device):
    """
    Extrait les features globales (MegaLoc) et les patch features (DINOv2).

    Returns:
        global_features: Features MegaLoc normalisées [1, 8448]
        patch_features: Patch features DINOv2 [1, 768, H//14, W//14]
    """
    with torch.no_grad():
        ...
        # Extract patch features from backbone (before aggregation)
        patch_features, cls_token = model.backbone(img_tensor)

        # Extract global features (full pipeline)
        global_features = model(img_tensor)

        return global_features.cpu(), patch_features.cpu()
...
def find_patch_matches(query_patches, loop_patches, top_k=20):
    """
    Trouve les meilleurs matches entre les patches de deux images.

    Args:
        query_patches: [1, C, H, W]
        loop_patches: [1, C, H, W]
    ...
    C, H, W = query_patches.shape[1], query_patches.shape[2], query_patches.shape[3]
    query_flat = query_patches.squeeze(0).reshape(C, -1).T  # [H*W, C]
    loop_flat = loop_patches.squeeze(0).reshape(C, -1).T    # [H*W, C]
...
def visualize_matches(query_img, loop_img, matches, grid_size, similarity_score, output_path):
    ...
    for (qx, qy), (lx, ly), sim in matches:
        ...
        # Draw connecting line from center to center
        ...
        line = plt.Line2D([coord1[0], coord2[0]], [coord1[1], coord2[1]],
                         transform=fig.transFigure, color=color, alpha=0.7,
    ...
    cbar.set_label('Patch Similarity', fontsize=12)
...
def main():
    parser = argparse.ArgumentParser(description="Loop Closure Detection with Visual Matches")
    parser.add_argument("--images_path", type=str, required=True,
                       help="Path to images directory")
    parser.add_argument("--output_dir", type=str, default="loop_matches",
    ...
    print("="*80)
    print("🎯 LOOP CLOSURE DETECTION WITH VISUAL MATCHES")
    print("="*80)
    ...
    for frame_idx, img_path in enumerate(image_files):
        ...
        # Extract features
        global_feat, patch_feat = extract_features_and_patches(model, img_tensor, device)
        all_global_features.append(global_feat)
        all_patch_features.append(patch_feat)
        ...
        if frame_idx >= args.start_detection_frame:
            ...
            if len(top_indices) > 0:
                ...
                if best_similarity >= args.similarity_threshold:
                    ...
                    # Find patch matches
                    print(f"   Finding {args.num_patch_matches} best patch matches...")
                    matches, grid_size = find_patch_matches(
                        all_patch_features[frame_idx],
                        all_patch_features[match_idx],
